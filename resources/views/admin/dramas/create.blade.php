...
@section('title', 'Add New Drama - KASA')
...
<div class="max-w-4xl mx-auto">
    <div class="mb-4 md:mb-6">
        ...
        <h1 class="text-2xl md:text-3xl font-bold">Add New Drama</h1>
    </div>

    <div class="bg-zinc-800 p-4 md:p-6 rounded-lg">
        <form action="{{ route('admin.dramas.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-4">
                ...
                <textarea id="description" 
                          ...
                          rows="5"
                          required
                          class="w-full px-4 py-2 rounded bg-zinc-900 text-white border border-zinc-700 focus:border-red-500 focus:outline-none">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
